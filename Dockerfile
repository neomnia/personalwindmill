...
RUN if [ "$WITH_POWERSHELL" = "true" ]; then \
    ...
    && rm -rf /var/lib/apt/lists/* && dpkg --install 'pwsh.deb' && rm 'pwsh.deb'; \
    ...
    && rm -rf /var/lib/apt/lists/* && mkdir -p /opt/microsoft/powershell/7 && tar zxf powershell.tar.gz -C /opt/microsoft/powershell/7 && chmod +x /opt/microsoft/powershell/7/pwsh && ln -s /opt/microsoft/powershell/7/pwsh /usr/bin/pwsh && rm powershell.tar.gz; \
    else echo 'Could not install pwsh, not on amd64 or arm64'; fi; \
...
RUN if [ "$WITH_HELM" = "true" ]; then \
    ...
    tar -zxvf "helm-v${HELM_VERSION}-linux-$arch.tar.gz" && mv linux-$arch/helm /usr/local/bin/helm && chmod +x /usr/local/bin/helm; \
    else echo 'Building the image without helm'; fi

# Install kubectl
RUN if [ "$WITH_KUBECTL" = "true" ]; then \
    arch="$(dpkg --print-architecture)"; arch="${arch##*-}"; \
    curl -LO "https://dl.k8s.io/release/v${KUBECTL_VERSION}/bin/linux/$arch/kubectl" && install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl; \
...
RUN set -eux; \
    arch="$(dpkg --print-architecture)"; arch="${arch##*-}"; \
    case "$arch" in \
    "amd64") targz="go${GO_VERSION}.linux-amd64.tar.gz";; \
    "arm64") targz="go${GO_VERSION}.linux-arm64.tar.gz";; \
    "armhf") targz="go${GO_VERSION}.linux-armv6l.tar.gz";; \
...
# Install UV
RUN curl -sL https://github.com/astral-sh/uv/releases/download/0.6.2/uv-installer.sh | sh && mv /root/.local/bin/uv /usr/local/bin/uv
...
RUN apt-get -y update && apt-get install -y curl procps nodejs awscli && apt-get clean && rm -rf /var/lib/apt/lists/*
...
# Create a non-root user 'windmill'
RUN addgroup --gid 1000 windmill && adduser --disabled-password --gecos "" --uid 1000 --gid 1000 windmill
RUN cp -r /root/.cache /home/windmill/.cache

RUN mkdir -p /tmp/windmill/logs && mkdir -p /tmp/windmill/search
RUN chmod -R 777 ${APP} && chmod -R 777 /tmp/windmill && chmod -R 777 /home/windmill/.cache
